...
Processor.prototype.parseNext = function() {
    ...
    if (this.options.commentEscape && line.trim() === '#')
        this.addLine(this.addDefines(line));

    // Else: unknown processor type
    else
        return this.error(`unknown processer type ${line}`)
...
const compilerNameRegex = /^__(TIME)|(DATE)|(LINE)|(FILE)__$/;
// Create a constant
Processor.prototype.createConstant =
    Compiler.prototype.createConstant = function(name, value, addDefines) {
        ...
        if( !compilerNameRegex.test(name) && this.defines[name] && this.defines[name].value !== value)
            this.error(`redefine macro "${name}" with different value "${value}"`)
...
createDirective("define", function(text) {
    ...
    if (name) {
        ...
        // Else: create a constant
            ...
            if (!regex.test(text) && !this.options.ignoreConstantError)
                return this.error(`informal constant macro ${text}`);
...
createDirective("if", function(expr) {
    ...
    expr = expr.replace(/defined\s*\(\s*([\w]+)\s*\)/g, function(match, p1) {
        if (typeof p1 === 'number')
            return this.error(`error defined expression:"${match}"`);
